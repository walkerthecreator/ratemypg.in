  ...
  return (
    <>
      ...
      <main
        ...
        id="magic"
      >
        <h1 className="text-6xl font-extrabold font-satoshi text-transparent bg-clip-text bg-gradient-to-r from-white via-zinc-200 to-zinc-400">
          Search your Accomodation
        </h1>
        <p className="mt-5 font-semibold text-zinc-300 font-satoshi">
          Add or Read reviews about your Pg/Room{" "}
        </p>
        ...
        <div className="w-2/4 flex justify-center mt-10">
          <div>
            <input
              ...
              type="text"
              className="border rounded-full p-1 px-4 bg-zinc-800 w-full"
                ...
                <ul className="p-2 px-4 flex flex-col items-center no-scrollbar">
                  {suggestions.map((item, index) => {
                    return (
                      <>
                        <li
                          className="p-1 my-1 hover:text-blue-500 transition-colors"
                          key={item?._id}
                        >
                          <Link href={`/pg/${item.name}`}>{item.name}</Link>
                        </li>
          ...
          <button className="bg-blue-700 p-1 px-4 text-white rounded-full ms-2">
            search
          </button>
        </div>
      </main>
